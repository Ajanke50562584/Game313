...
// These paths match the prefixes and filenames declared in Resources.qrc.
    ...
    : QWidget(parent)
{
    ...
    // The background image stretches with the widget through Qt style sheets.
    ...
    // Load the sprite sheet from the Qt resource system and split it into frames.
    ...
    {
        const int Frame_Width = Sprite_Sheet->width() / 8;
        const int Frame_Height = Sprite_Sheet->height();

        if(Frame_Width > 0 && Frame_Height > 0)
        {
            // The sitting animation sheet contains eight frames in one row.
    ...
    // This label is raised above the grid so the character appears over the background.
    ...
    // The grid makes the diagonal button placement responsive.
    ...
    // Keep the visual flow from top-left, to center, to bottom-right.
...
void Menu_Multiplayer_Screen::Apply_Responsive_Sizing()
{
    ...
    {
        // Anchor the character near the bottom-left while still scaling it with the window.
