...
// Background path declared in Resources.qrc.
    ...
    : QWidget(parent)
{
    ...
    setStyleSheet(QString(
                      ...
                      "}").arg(":/Images/Background/Space_BG_4.png"));
    ...
    Card_Widget->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Maximum);
    ...
    Lobby_Name_Validator = new QRegularExpressionValidator(
                QRegularExpression("^[A-Za-z0-9 _\\-]{0,16}$"),
                Lobby_Name_Input);
    ...
    // also allow _
    // also allow -
    // length must be from 0 to 16 characters
    ...
    connect(Public_Lobby_Checkbox, SIGNAL(toggled(bool)), this, SLOT(Handle_Public_Lobby_Toggled(bool)));// make pasword feild greyd / not greyed out
    connect(Show_Password_Checkbox, SIGNAL(toggled(bool)), this, SLOT(Handle_Show_Password_Toggled(bool)));
    connect(Host_Name_Input, SIGNAL(textChanged(QString)), this, SLOT(Handle_Host_Name_Changed(QString))); // If host name changed let the button to create lobby become usable
...
void Menu_Create_Lobby_Screen::resizeEvent(QResizeEvent *event)
{
    const int w = width();
    const int h = height();
    const int Font_Base = qMin(w, (h * 16) / 9);

    Main_Layout->setContentsMargins(qMax(12, (24 * w) / 1280),
                                    qMax(12, (24 * h) / 720),
                                    qMax(12, (24 * w) / 1280),
                                    qMax(12, (24 * h) / 720));
    Main_Layout->setSpacing(qMax(6, (12 * h) / 720));

    Card_Widget->setMaximumWidth(qMin(qMax(260, w - 40), qMax(300, (560 * w) / 1280)));
    Card_Layout->setContentsMargins(qMax(14, (26 * w) / 1280),
                                    qMax(10, (20 * h) / 720),
                                    qMax(14, (26 * w) / 1280),
                                    qMax(10, (20 * h) / 720));
    Card_Layout->setSpacing(qMax(5, (10 * h) / 720));

    // scale text with the screen width
    QFont Title_Font;
    Title_Font.setPointSize(qMax(16, Font_Base / 46));
    Title_Label->setFont(Title_Font);

    QFont Sub_Font;
    Sub_Font.setPointSize(qMax(9, Font_Base / 92));
    Sub_Title_Label->setFont(Sub_Font);

    QFont Input_Font;
    Input_Font.setPointSize(qMax(8, Font_Base / 105));
    ...
    const int Button_Height = qMax(34, (46 * h) / 720);
