...
bool Game_Networking_Elements::Start_Client_Listening()
{
    ...
    bool ok = Game_UDP_Socket->bind(QHostAddress::AnyIPv4,
                                    LOBBY_PORT,
                                    QUdpSocket::ShareAddress | QUdpSocket::ReuseAddressHint);

    if(!ok)
    {
        qDebug() << "client bind to lobby port failed, falling back to any port:"
                 << Game_UDP_Socket->errorString();

        // use any free port if the shared lobby port is already taken
        ok = Game_UDP_Socket->bind(QHostAddress::AnyIPv4,
                                   0,
                                   QUdpSocket::ShareAddress | QUdpSocket::ReuseAddressHint);
    }
