    ...
    : QWidget(parent)
{
    ...
    Select_Level_Button = new QPushButton("Start Level 1", this);
    ...
    Level_Grid_Widget = new QWidget(this);
    Level_Grid_Widget->setStyleSheet("background: transparent; border: none;");
    Level_Grid_Layout = new QGridLayout(Level_Grid_Widget);
    Level_Grid_Layout->setContentsMargins(0, 0, 0, 0);
    Level_Grid_Layout->setSpacing(8);

    for(int i = 1; i <= 10; i++)
    {
        QPushButton *Level_Button = new QPushButton("L" + QString::number(i), this);
        Level_Button->setProperty("levelNumber", i);
        Level_Buttons.append(Level_Button);
        Buttons.append(Level_Button);
        Level_Grid_Layout->addWidget(Level_Button, (i - 1) / 5, (i - 1) % 5);
    }
    ...
    Main_Layout->addWidget(Tutorial_Button, 0, Qt::AlignHCenter);
    Main_Layout->addWidget(Select_Level_Button, 0, Qt::AlignHCenter);
    Main_Layout->addWidget(Level_Grid_Widget, 0, Qt::AlignHCenter);
    Main_Layout->addWidget(Back_Button, 0, Qt::AlignHCenter);
...
QList<QPushButton*> Story_Mode_Main_Screen::Get_Level_Buttons() const
{
...
void Story_Mode_Main_Screen::Apply_Responsive_Sizing()
{
    ...
    Push_Button_Font.setPointSize(qMax(8, w / 82));

    const int Button_Height = qMax(32, (42 * h) / 720);
    const int Button_Max_Width = qMax(150, (330 * w) / 1280);
    const int Level_Button_Width = qMax(80, (115 * w) / 1280);
    ...
    {
        Level_Buttons[i]->setMinimumWidth(Level_Button_Width);
        Level_Buttons[i]->setMaximumWidth(Level_Button_Width);
    }

    if(Character_Label != nullptr)
    {
        // Keep the character anchored near the bottom-left corner.
        Character_Label->resize(qMax(84, (260 * w) / 1280),
                                qMax(84, (260 * h) / 720));
