    ...
    : QOpenGLWidget(parent)
{
    ...
    Start_Multiplayer_Level("Medium");
}

void Multiplayer_Game_Screen::Start_Multiplayer_Level(const QString &difficulty)
{
    Current_Level.Load_Multiplayer_Level(difficulty);
    Course_Timer.restart();
    Camera_X = 0;
    Camera_Y = 0;
    Final_Time_Ms = 0;
    Time_Penalty_Ms = 0;
    Enemy_Hit_Cooldown_Ticks = 0;
    Course_Running = true;
...
void Multiplayer_Game_Screen::Set_Run_Pressed(bool pressed)
{
    Local_Player.Set_Run_Pressed(pressed);
}
...
void Multiplayer_Game_Screen::Draw_Round_Result_Overlay(QPainter &painter)
{
    painter.save();
    painter.setRenderHint(QPainter::Antialiasing, true);
    ...
    painter.setFont(Title_Font);
    painter.setPen(QColor(235,235,245));
    painter.drawText(Panel.adjusted(18, 16, -18, -310), Qt::AlignCenter, "Round Leaderboard");
    ...
    QString Text;
    Text += "Run time: " + Leaderboard_Data::Format_Time(Final_Time_Ms - Time_Penalty_Ms) + "\n";
    Text += "Penalty: " + Leaderboard_Data::Format_Time(Time_Penalty_Ms) + "\n";
    Text += "Total: " + Leaderboard_Data::Format_Time(Final_Time_Ms) + "\n\n";
    ...
    Text += "\nHost can start the next round from the lobby.";
...
void Multiplayer_Game_Screen::Draw_Hud(QPainter &painter)
{
    ...
    if(Course_Running == true)
        Time_Ms = (int)Course_Timer.elapsed() + Time_Penalty_Ms;
    ...
    painter.drawText(Timer_Rect.adjusted(12, 0, -12, 0),
                     ...
                     "   Penalty: " + Leaderboard_Data::Format_Time(Time_Penalty_Ms));
...
void Multiplayer_Game_Screen::Update_Game()
{
    ...
    if(Enemy_Hit_Cooldown_Ticks > 0)
        Enemy_Hit_Cooldown_Ticks--;
...
void Multiplayer_Game_Screen::Check_Win_Lose_Conditions()
{
    ...
    {
        Final_Time_Ms = (int)Course_Timer.elapsed() + Time_Penalty_Ms;
    ...
    for(int i = 0; i < Hazards.size(); i++)
    {
        ...
        {
            Reset_Player_To_Chunk_Start();
            return;
        }
    }

    QVector<QPointF> Enemies = Current_Level.Get_Enemy_Spawns();
    for(int i = 0; i < Enemies.size(); i++)
    {
        QRectF Enemy_Rect(Enemies[i].x(), Enemies[i].y(),
                          Current_Level.Get_Tile_Size(),
                          Current_Level.Get_Tile_Size());

        if(Player_Rect.intersects(Enemy_Rect))
        {
            Apply_Enemy_Time_Penalty();
            return;
        }
    }
...
void Multiplayer_Game_Screen::Reset_Player_To_Chunk_Start()
{
    Local_Player.Set_Player_Spawn(Current_Level.Get_Player_Spawn());
}

void Multiplayer_Game_Screen::Apply_Enemy_Time_Penalty()
{
    if(Enemy_Hit_Cooldown_Ticks > 0)
        return;

    Time_Penalty_Ms += 5000;
    Enemy_Hit_Cooldown_Ticks = 90;
}
