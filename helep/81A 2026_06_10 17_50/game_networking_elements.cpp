    ...
    : QObject(parent)
{
    ...
    // repeats host announcements while this player is hosting
    Lobby_Announce_Timer = new QTimer(this);
    Lobby_Announce_Timer->setInterval(1000);

    // read packets as they arrive
    connect(Game_UDP_Socket, SIGNAL(readyRead()), this, SLOT(Handle_Pending_Datagrams()));

    // send the host lobby info every time the timer ticks
    connect(Lobby_Announce_Timer, &QTimer::timeout, this,&Game_Networking_Elements::Send_Lobby_Announce_Broadcasts);
}
...
bool Game_Networking_Elements::Start_Listening()
{
    ...
    const bool ok = Game_UDP_Socket->bind(QHostAddress::AnyIPv4,LOBBY_PORT,QUdpSocket::ShareAddress | QUdpSocket::ReuseAddressHint);

    if(!ok)
        qDebug() << "host bind failed:" << Game_UDP_Socket->errorString();

    if(ok)
    {
        Connected_Client_Endpoints.clear();

        // announce right away and keep announcing while hosting
        Send_Lobby_Announce_Broadcasts();
        Lobby_Announce_Timer->start();
    }
    else if(Lobby_Announce_Timer != nullptr)
    {
        Lobby_Announce_Timer->stop();
    }

    if(Shared_Group != nullptr)
    {
        // keep shared labels in sync with socket state
        Shared_Group->Set_Connection_Status(ok ? "Listening on port " + QString::number(LOBBY_PORT): "Failed to bind port " + QString::number(LOBBY_PORT));

        Shared_Group->Set_Remote_Address(ok ? "0.0.0.0:" + QString::number(LOBBY_PORT): "Not connected");

        Shared_Group->Set_Last_Network_Event(ok ? "socket bound": "bind failed");
    }
...
bool Game_Networking_Elements::Start_Client_Listening()
{
    ...
    // stop host announcements when switching to client mode
    if(Lobby_Announce_Timer != nullptr)
        Lobby_Announce_Timer->stop();

    // try the shared lobby port first so host broadcasts can be received
    bool ok = Game_UDP_Socket->bind(QHostAddress::AnyIPv4,LOBBY_PORT,QUdpSocket::ShareAddress | QUdpSocket::ReuseAddressHint);

    if(!ok)
    {
        qDebug() << "client bind to lobby port failed, falling back to any port:" << Game_UDP_Socket->errorString();

        // use any free port if the shared lobby port is already taken
        ok = Game_UDP_Socket->bind(QHostAddress::AnyIPv4,0, QUdpSocket::ShareAddress | QUdpSocket::ReuseAddressHint);
    }

    if(Shared_Group != nullptr)
    {
        Shared_Group->Set_Connection_Status(ok ? "Client listening": "Failed to bind client socket");

        Shared_Group->Set_Remote_Address(ok ? "Client reply port ready": "Not connected");

        Shared_Group->Set_Last_Network_Event(ok ? "client socket bound" : "client bind failed");
    }
...
void Game_Networking_Elements::Send_Discover_Lobbies()
{
    ...
    // send to the normal broadcast address first
    QSet<QString> Sent_Broadcasts;
    Sent_Broadcasts.insert(QHostAddress(QHostAddress::Broadcast).toString());
    qint64 Written = Game_UDP_Socket->writeDatagram(data, QHostAddress::Broadcast, LOBBY_PORT);
    if(Written == -1)
        qDebug() << "failed to send discover to broadcast:" << Game_UDP_Socket->errorString();

    // also send to each network adapter's real broadcast address
    const QList<QNetworkInterface> Interfaces = QNetworkInterface::allInterfaces();
    for(int i = 0; i < Interfaces.size(); ++i)
    {
        const QNetworkInterface Interface = Interfaces[i];

        // skip inactive adapters and loopback
        ...
        {
            const QHostAddress Broadcast = Entries[j].broadcast();

            // only use valid IPv4 broadcast addresses
            if(Broadcast.isNull() || Broadcast.protocol() != QAbstractSocket::IPv4Protocol)
                continue;

            const QString Broadcast_Text = Broadcast.toString();

            // avoid sending twice to the same broadcast address
            if(Sent_Broadcasts.contains(Broadcast_Text))
                continue;

            Sent_Broadcasts.insert(Broadcast_Text);
            Written = Game_UDP_Socket->writeDatagram(data, Broadcast, LOBBY_PORT);
            if(Written == -1)
                qDebug() << "failed to send discover to" << Broadcast_Text << Game_UDP_Socket->errorString();
            else
                qDebug() << "sent discover to" << Broadcast_Text;
        }
    }
...
void Game_Networking_Elements::Send_Lobby_Announce(const QHostAddress &address, quint16 port)
{
    ...
    obj["mapDifficulty"] = Shared_Group->Get_Map_Difficulty();
    obj["isPublicLobby"] = Shared_Group->Get_Is_Public_Lobby();

    QJsonDocument doc(obj);
    QByteArray data = doc.toJson(QJsonDocument::Compact);

    const qint64 Written = Game_UDP_Socket->writeDatagram(data, address, port);

    if(Written == -1)
        qDebug() << "failed to send lobby announce to" << address.toString() << port << Game_UDP_Socket->errorString();
    else
...
void Game_Networking_Elements::Process_JSON_Packet(const QJsonObject &obj, const QHostAddress &sender, quint16 senderPort)
{
    ...
    if(type == "DISCOVER_LOBBIES")
    {
        ...
        if(Shared_Group->Get_Is_Hosting())
        {
            ...
            // also reply to the shared lobby port if the client used a random port
            if(senderPort != LOBBY_PORT)
                Send_Lobby_Announce(sender, LOBBY_PORT);
        }
        else
        {
    ...
    else if(type == "LOBBY_ANNOUNCE")
    {
        ...
        lobby.Map_Difficulty = obj["mapDifficulty"].toString("Medium");
        lobby.Is_Public_Lobby = obj["isPublicLobby"].toBool(true);

        // store this lobby or refresh the old entry
        Shared_Group->Add_Discovered_Lobby(lobby);

        if(Shared_Group->Get_Is_In_Lobby() && !Shared_Group->Get_Is_Hosting())
            Shared_Group->Set_Map_Difficulty(lobby.Map_Difficulty);
    }
    ...
    else if(type == "GAME_START")
    {
        ...
        Shared_Group->Set_Map_Difficulty(obj["mapDifficulty"].toString("Medium"));
        emit Game_Start_Received();
    }
    else if(type == "MAP_VOTE")
    {
        const QString Difficulty = obj["difficulty"].toString("Medium");
        Shared_Group->Add_Map_Difficulty_Vote(Difficulty);
        qDebug() << "MAP_VOTE received:" << Difficulty;
    }
...
void Game_Networking_Elements::Send_Map_Vote(const QString &difficulty)
{
    if(Game_UDP_Socket == nullptr || Shared_Group == nullptr)
        return;
...
void Game_Networking_Elements::Send_Lobby_Announce_Broadcasts()
{
    if(Game_UDP_Socket == nullptr || Shared_Group == nullptr || !Shared_Group->Get_Is_Hosting())
        return;

    QSet<QString> Sent_Broadcasts;

    // send to the normal broadcast address first
    Send_Lobby_Announce(QHostAddress::Broadcast, LOBBY_PORT);
    Sent_Broadcasts.insert(QHostAddress(QHostAddress::Broadcast).toString());
    ...
    for(int i = 0; i < Interfaces.size(); ++i)
    {
        ...
        for(int j = 0; j < Entries.size(); ++j)
        {
            ...
            Sent_Broadcasts.insert(Broadcast_Text);
            Send_Lobby_Announce(Broadcast, LOBBY_PORT);
        }
    }
}
