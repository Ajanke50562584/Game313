...
static const char *PLAYER_RUN_PATH = ":/Images/Characters/Running_Gray.png";
static const char *PLAYER_RUN_LEFT_PATH = ":/Images/Characters/Running_Left_Gray.png";
...
void Player::Load_Story_Mode_Images()
{
    ...
    Player_Run_Image = QImage(PLAYER_RUN_PATH);
    Player_Run_Left_Image = QImage(PLAYER_RUN_LEFT_PATH);

    if(Player_Run_Image.isNull())
        Player_Run_Image = Player_Walk_Image;

    if(Player_Run_Left_Image.isNull())
        Player_Run_Left_Image = Player_Walk_Left_Image;
...
void Player::Draw_Story_Mode_Player(QPainter &painter,float cameraX,float cameraY,float scale,float viewOffsetX,float viewOffsetY)
{
    ...
    //fisika
    QRectF targetRect(viewOffsetX + (Player_Physics_Body.Get_X_Pos() - cameraX - (STORY_MODE_PLAYER_WORLD_SIZE * 0.20f)) * scale,viewOffsetY + (Player_Physics_Body.Get_Bottom() - cameraY - STORY_MODE_PLAYER_WORLD_SIZE) * scale,STORY_MODE_PLAYER_WORLD_SIZE * scale,STORY_MODE_PLAYER_WORLD_SIZE * scale); //actual drawn rect on screen
...
{
    //fisika
    const float Standing_Width = STORY_MODE_PLAYER_WORLD_SIZE * 0.42f;
    const float Standing_Height = STORY_MODE_PLAYER_WORLD_SIZE * 0.48f;
    const float Crouch_Height = STORY_MODE_PLAYER_WORLD_SIZE * 0.24f;
    const float Old_Bottom = Player_Physics_Body.Get_Bottom();
    const bool Wants_To_Crouch = Move_Crouch_Pressed == true || (Player_Crouched == true && Can_Stand_Up == false);

    if(Wants_To_Crouch == true)
    {
        Player_Physics_Body.Set_Size(Standing_Width, Crouch_Height);
        Player_Physics_Body.Set_Pos(Player_Physics_Body.Get_X_Pos(),
                                    Old_Bottom - Crouch_Height);
        Player_Crouched = true;
    }
    else
    {
        Player_Physics_Body.Set_Size(Standing_Width, Standing_Height);
        Player_Physics_Body.Set_Pos(Player_Physics_Body.Get_X_Pos(),
                                    Old_Bottom - Standing_Height);
        Player_Crouched = false;
    }

    // left / right movement
    const float Walk_Speed = 850;
    const float Run_Speed = 1200;
    const float Move_Speed = Run_Pressed == true ? Run_Speed : Walk_Speed;
    ...
    else if (Move_Climb_Pressed == 1) //tests if busy climbing
    {
    ...
    else if (Move_Left_Pressed == 1)
    {
        if(Run_Pressed == true)
        {
            ...
            Current_Player_Image = &Player_Run_Left_Image;
        }
        else
        {
    ...
    else if (Move_Right_Pressed == 1)
    {
        if(Run_Pressed == true)
        {
            ...
            Current_Player_Image = &Player_Run_Image;
        }
        else
        {
    ...
    int Current_Animation_Speed = Animation_Speed;

    if(Current_Animation == Player_Climb)
    {
        Current_Animation_Speed = Animation_Speed * 2;
    }

    if (Animation_Tick >= Current_Animation_Speed)
    {
        Animation_Tick = 0;
...
void Player::Set_Can_Stand_Up(bool canStandUp)
{
    Can_Stand_Up = canStandUp;
}
...
void Player::Set_Run_Pressed(bool pressed)
{
    Run_Pressed = pressed;
}
