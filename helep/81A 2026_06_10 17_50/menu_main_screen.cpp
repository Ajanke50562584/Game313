#include "menu_main_screen.h"

#include <QApplication>
#include <QFont>
#include <QHideEvent>
#include <QHBoxLayout>
#include <QLabel>
#include <QPushButton>
#include <QResizeEvent>
#include <QShowEvent>
#include <QSizePolicy>
#include <QVBoxLayout>
#include <QPixmap>
#include <QDebug>
#include <QTimer>

// These paths come straight from Resources.qrc.
// Qt resource paths always start with ":/" followed by the qresource prefix.
static const char *MAIN_MENU_BACKGROUND_PATH = ":/Images/Background/Space_BG_1.png";
static const char *MAIN_MENU_CHARACTER_PATH = ":/Images/Characters/Awake_Laying_Orange.png";

Menu_Main_Screen::Menu_Main_Screen(QWidget *parent)
    : QWidget(parent)
{
    Current_Frame = 0;
    Completed_Animation_Loops = 0;
    Animation_Status_Reported = false;

    // The vertical layout stacks the title, menu buttons, and empty space.
    Main_Layout = new QVBoxLayout(this);


    setAttribute(Qt::WA_StyledBackground, true);
    QPixmap test(MAIN_MENU_BACKGROUND_PATH);
    qDebug() << test.isNull();


    // Load the sprite sheet from the compiled Qt resource file.
    Sprite_Sheet = new QPixmap(MAIN_MENU_CHARACTER_PATH);
    qDebug() << "sprite null?" << Sprite_Sheet->isNull();
    qDebug() << "sprite size:" << Sprite_Sheet->width() << Sprite_Sheet->height();

    int frameWidth = 128;
    int frameHeight = 128;

    // Each animation frame is stored beside the next one in a horizontal strip.
    Frames.clear();
    for (int x = 0; x + frameWidth <= Sprite_Sheet->width(); x += frameWidth)
    {
        Frames.append(Sprite_Sheet->copy(x, 0, frameWidth, frameHeight));
    }
    qDebug() << "frames count:" << Frames.size();

    // Character_Label is moved manually so it can sit over the background.
    Character_Label = new QLabel(this);
    Character_Label->setAttribute(Qt::WA_TranslucentBackground);
    if (!Frames.isEmpty())
    {
        Character_Label->setPixmap(Frames[0]);
        qDebug() << "first frame size:" << Frames[0].size();
    }

    // The timer drives the sprite animation by calling Update_Frame repeatedly.
    Menu_Character_Timer = new QTimer(this);
    connect(Menu_Character_Timer, SIGNAL(timeout()), this, SLOT(Update_Frame()));
    Menu_Character_Timer->start(150);


    // The background image is pulled from Resources.qrc and stretches with the widget.
    setObjectName("MainMenu");
    setStyleSheet(QString(
                      "#MainMenu {"
                      "border-image: url(%1) 0 0 0 0 stretch stretch;"
                      "}").arg(MAIN_MENU_BACKGROUND_PATH));

    Game_Title = new QLabel("Astro-Cats: <br>  The Last Constellation", this);
    Game_Title->setAlignment(Qt::AlignCenter);
    Game_Title->setWordWrap(true);
    Game_Title->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);

    Story_Mode_Button = new QPushButton("Story Mode", this);
    Multiplayer_Button = new QPushButton("Multi Player", this);
    View_Highscore_Button = new QPushButton("View Highscores", this);
    QPushButton *Exit_Game_Button = new QPushButton("Exit Game", this);

    // The list lets the sizing and styling code treat every menu button the same way.
    Buttons.append(Story_Mode_Button);
    Buttons.append(Multiplayer_Button);
    Buttons.append(View_Highscore_Button);
    Buttons.append(Exit_Game_Button);

    // The final height is calculated later in Apply_Responsive_Sizing().
    for(int i = 0; i < Buttons.size(); i++)
        Buttons[i]->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Fixed);

    // Stretch on both sides keeps the title centered without hard-coded positions.
    QHBoxLayout *Title_Row = new QHBoxLayout();
    Title_Row->addStretch(1);
    Title_Row->addWidget(Game_Title, 3);
    Title_Row->addStretch(1);

    Main_Layout->addLayout(Title_Row);
    Main_Layout->addSpacing(20);
    Main_Layout->addStretch(1);

    // Add each button to the layout and connect simple cursor feedback slots.
    for(int i = 0; i < Buttons.size(); i++)
    {
        Main_Layout->addWidget(Buttons[i], 0, Qt::AlignHCenter);
        Buttons[i]->setCursor(Qt::PointingHandCursor);
        connect(Buttons[i], SIGNAL(pressed()), this, SLOT(Set_Button_Cursor_To_Closed_Hand()));
        connect(Buttons[i], SIGNAL(released()), this, SLOT(Set_Button_Cursor_To_Pointing_Hand()));
    }

    Main_Layout->addStretch(2);

    // Exit button closes the application.
    connect(Exit_Game_Button, SIGNAL(clicked()), qApp, SLOT(quit()));

    Apply_Responsive_Sizing();
}

QPushButton *Menu_Main_Screen::Get_Multiplayer_Button() const
{
    return Multiplayer_Button;
}

QPushButton *Menu_Main_Screen::Get_Story_Mode_Button() const
{
    return  Story_Mode_Button;
}

QPushButton *Menu_Main_Screen::Get_Highscores_Button() const
{
    return View_Highscore_Button;
}

void Menu_Main_Screen::resizeEvent(QResizeEvent *event)
{
    QWidget::resizeEvent(event);
    Apply_Responsive_Sizing();
}

void Menu_Main_Screen::showEvent(QShowEvent *event)
{
    QWidget::showEvent(event);

    if(Menu_Character_Timer != nullptr && !Menu_Character_Timer->isActive())
    {
        Menu_Character_Timer->start(150);
        qDebug() << "Animation started: Main menu";
    }
}

void Menu_Main_Screen::hideEvent(QHideEvent *event)
{
    QWidget::hideEvent(event);

    if(Menu_Character_Timer != nullptr && Menu_Character_Timer->isActive())
    {
        Menu_Character_Timer->stop();
        qDebug() << "Animation stopped: Main menu";
    }
}

void Menu_Main_Screen::Update_Frame()
{
    if (Frames.isEmpty())
    {
        Report_Animation_Error_Once();
        return;
    }

    // Scale the current frame to the label size so the cat grows and shrinks with the window.
    QPixmap scaled = Frames[Current_Frame].scaled(
                Character_Label->size(),
                Qt::KeepAspectRatio,
                Qt::FastTransformation
                );
    Character_Label->setPixmap(scaled);

    Current_Frame = (Current_Frame + 1) % Frames.size();

    if(Current_Frame == 0)
    {
        Completed_Animation_Loops++;

        if(Completed_Animation_Loops >= 3 && !Animation_Status_Reported)
        {
            Animation_Status_Reported = true;
            qDebug() << "Animation successful: Main menu";
        }
    }
}

void Menu_Main_Screen::Apply_Responsive_Sizing()
{
    const int w = width();
    const int h = height();

    // Margins and spacing are calculated from the current window size.
    Main_Layout->setContentsMargins(qMax(12, (24 * w) / 1280),
                                    qMax(12, (24 * h) / 720),
                                    qMax(12, (24 * w) / 1280),
                                    qMax(12, (24 * h) / 720));
    Main_Layout->setSpacing(qMax(8, (16 * h) / 720));

    QFont Title_Font("OCR A Extended");
    Title_Font.setPointSize(qMax(18, w / 40));
    Game_Title->setFont(Title_Font);

    QFont Push_Button_Font("OCR A Extended");
    Push_Button_Font.setPointSize(qMax(10, w / 65));

    const int Button_Height = qMax(40, (55 * h) / 720);
    const int Button_Max_Width = qMax(220, (500 * w) / 1280);

    // Buttons share the same responsive font and size rules.
    for(int i = 0; i < Buttons.size(); i++)
    {
        Buttons[i]->setFont(Push_Button_Font);
        Buttons[i]->setMinimumHeight(Button_Height);
        Buttons[i]->setMaximumWidth(Button_Max_Width);
    }

    if(Character_Label != nullptr)
    {
        // The character is anchored near the bottom-left corner using scaled offsets.
        Character_Label->resize(qMax(96, (384 * w) / 1280),
                                qMax(96, (384 * h) / 720));
        Character_Label->move(qMax(12, (40 * w) / 1280),
                              height() - Character_Label->height() - qMax(12, (40 * h) / 720));
        Character_Label->raise();
    }

    Apply_Responsive_Styles();
}

void Menu_Main_Screen::Apply_Responsive_Styles()
{
    const int w = qMax(width(), 640);
    const int h = qMax(height(), 360);
    const int Button_Radius = qMax(14, (26 * h) / 720);
    const int Button_Padding_V = qMax(6, (10 * h) / 720);
    const int Button_Padding_H = qMax(14, (24 * w) / 1280);
    const int Title_Radius = qMax(12, (18 * h) / 720);
    const int Title_Padding_V = qMax(6, (10 * h) / 720);
    const int Title_Padding_H = qMax(12, (20 * w) / 1280);

    for(int i = 0; i < Buttons.size(); i++)
    {
        Buttons[i]->setStyleSheet(QString(
                                      "QPushButton {"
                                      "background-color: rgba(210,210,220,135);"
                                      "color: rgb(20,20,30);"
                                      "border: 2px solid rgba(245,245,255,190);"
                                      "border-radius: %1px;"
                                      "padding: %2px %3px;"
                                      "}"
                                      "QPushButton:hover {"
                                      "background-color: rgba(235,235,245,175);"
                                      "border: 2px solid rgba(255,255,255,230);"
                                      "}"
                                      "QPushButton:pressed {"
                                      "background-color: rgba(180,180,195,190);"
                                      "}")
                                  .arg(Button_Radius)
                                  .arg(Button_Padding_V)
                                  .arg(Button_Padding_H));
    }

    Game_Title->setStyleSheet(QString(
                                  "color: rgb(235,235,245);"
                                  "background-color: rgba(0, 74, 113, 237);"
                                  "border: 2px solid rgba(200,200,230,120);"
                                  "border-radius: %1px;"
                                  "padding: %2px %3px;")
                              .arg(Title_Radius)
                              .arg(Title_Padding_V)
                              .arg(Title_Padding_H));
}

void Menu_Main_Screen::Report_Animation_Error_Once()
{
    if(Animation_Status_Reported)
        return;

    Animation_Status_Reported = true;
    qDebug() << "Animation error: Main menu";
}

void Menu_Main_Screen::Set_Button_Cursor_To_Closed_Hand()
{
    QPushButton *Button = qobject_cast<QPushButton*>(sender());

    if(Button != nullptr)
        Button->setCursor(Qt::ClosedHandCursor);
}

void Menu_Main_Screen::Set_Button_Cursor_To_Pointing_Hand()
{
    QPushButton *Button = qobject_cast<QPushButton*>(sender());

    if(Button != nullptr)
        Button->setCursor(Qt::PointingHandCursor);
}
