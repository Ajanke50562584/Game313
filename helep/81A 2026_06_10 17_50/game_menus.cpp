    ...
    : QObject(parent)
{
    ...
    Story_Level_Buttons = Story_Mode_Main_Menu_Screen->Get_Level_Buttons();
...
void Game_Menus::Setup_Navigation()
{
    ...
    for(int i = 0; i < Story_Level_Buttons.size(); i++)
    {
        QPushButton *Level_Button = Story_Level_Buttons[i];
        connect(Level_Button, SIGNAL(clicked()), this, SLOT(Story_Level_Button_Clicked()));
    }
...
void Game_Menus::Go_To_Multiplayer_Game()
{
    ...
    if(Shared_Group != nullptr)
        Difficulty = Shared_Group->Get_Map_Difficulty();

    Multiplayer_Game_Screen_Widget->Start_Multiplayer_Level(Difficulty);
...
void Game_Menus::Select_Level_Button_Clicked()
{
    ...
    Single_Player_Game_Screen_Widget->Start_Story_Level_Number(1);
    Go_To_Single_Player_Game();
}

void Game_Menus::Story_Level_Button_Clicked()
{
    QPushButton *Level_Button = qobject_cast<QPushButton*>(sender());

    if(Level_Button == nullptr)
        return;

    const int Level_Number = Level_Button->property("levelNumber").toInt();
    const int Safe_Level_Number = qBound(1, Level_Number, 10);

    qDebug() << "Starting story level" << Safe_Level_Number;
    Single_Player_Game_Screen_Widget->Start_Story_Level_Number(Safe_Level_Number);
    Go_To_Single_Player_Game();
}
