...
//                      Background_Image);
    ...
    QRect targetRect(Player_World_X - Camera_X,
                     Player_World_Y - Camera_Y,
                     playerDrawSize,
                     playerDrawSize);
    //gl again only draw
...
void Story_Mode_Tutorial::Update_Game()
{
    ...
    // update position
    Player_World_X += Player_Vx * dt;
    Player_World_Y += Player_Vy * dt;

    //makes sure the camera is following the character
    Camera_X = Player_World_X - width() * 0.35f;
...
//        Camera_X = Max_Camera_X;
    ...
    if (Player_World_Y + playerDrawSize > platformY)
    {
        Player_World_Y = platformY - playerDrawSize;
        Player_Vy = 0;
