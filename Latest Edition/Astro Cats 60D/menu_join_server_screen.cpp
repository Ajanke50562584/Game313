...
static const char *JOIN_SERVER_BACKGROUND_PATH = ":/Images/Background/Space_BG_3.png";
    ...
    : QWidget(parent)
{
    ...
    setStyleSheet(QString(
                      ...
                      "}").arg(JOIN_SERVER_BACKGROUND_PATH));
    ...
    Player_Name_Validator = new QRegularExpressionValidator(
                QRegularExpression("^[A-Za-z0-9 _\\-]{0,24}$"), Player_Name_Input); /*A-Z
                                                                                    a-z
                                                                                    0-9
                                                                                    space
                                                                                    underscore _
    ...
    // Consistant buttons cuz thats awesome
    ...
    //Makes navgation game menu's problem
    connect(Join_Button, SIGNAL(clicked()), this, SLOT(Handle_Join_Button_Clicked()));
    connect(Refresh_Button, SIGNAL(clicked()), this, SIGNAL(Refresh_Button_Clicked()));
    ...
    Title_Row->addWidget(Title_Label, 3); // 3/5ths to stay consistant with other layout cards
    Title_Row->addStretch(1);

    QHBoxLayout *Card_Row = new QHBoxLayout();
    Card_Row->addStretch(1);
    Card_Row->addWidget(Card_Widget, 0, Qt::AlignHCenter);// add layout to row
    Card_Row->addStretch(1);
...
void Menu_Join_Server_Screen::Set_Lobby_List(const QStringList &lobbyTexts)
{
    Lobby_List->clear(); // empty current list
    Lobby_List->addItems(lobbyTexts);

    if(Lobby_List->count() > 0)
        Lobby_List->setCurrentRow(0);
}
...
void Menu_Join_Server_Screen::resizeEvent(QResizeEvent *event)
{
    ...
    Main_Layout->setContentsMargins(qMax(12, (24 * w) / 1280), // take biggest bewtween 12 and the second expressions result
                                    qMax(12, (24 * h) / 720),
                                    qMax(12, (24 * w) / 1280),
                                    qMax(12, (24 * h) / 720));
    ...
    // Apply the fonts to the buttons ect
...
void Menu_Join_Server_Screen::Apply_Responsive_Styles()
{
    ...
    Title_Label->setStyleSheet(QString(
                                   ...
                                   "font-weight: 700;"
                                   "font-family: 'OCR A Extended';")
                               .arg(Title_Radius).arg(Title_Pad_V).arg(Title_Pad_H)); // need to use arg notation for dynamic runtime resizing
...
void Menu_Join_Server_Screen::Handle_Join_Button_Clicked()
{
    QString Player_Name = Player_Name_Input->text().trimmed();

    emit Join_Button_Clicked(Lobby_List->currentRow(), Player_Name); // Emit the join signal
}
