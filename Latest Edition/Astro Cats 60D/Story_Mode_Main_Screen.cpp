    ...
    : QWidget(parent)
{
    ...
    // The timer to animate black cat
    ...
    setStyleSheet(QString(
                      "#MainMenu {"
                      "border-image: url(%1) 0 0 0 0 stretch stretch;" // stretch both horz and vertical
    ...
    Title_Row->addWidget(Story_Mode_Title, 3); // Title gets 3/5ths of the top
    Title_Row->addStretch(1);

    Main_Layout->addLayout(Title_Row);
    Main_Layout->addSpacing(20); // 20 px between each row
    Main_Layout->addStretch(1);


    // Add buttons and connect muis pointer
    for(int i = 0; i < Buttons.size(); i++)
    {
...
void Story_Mode_Main_Screen::Apply_Responsive_Styles()
{
    ...
    for(int i = 0; i < Buttons.size(); i++)
    {
        Buttons[i]->setStyleSheet(QString(
                                      ...
                                      "}")
                                  .arg(Button_Radius) // need this for the dynamic updating T_T
