...
#include <QHostAddress> // To store the local IP
#include <QJsonObject>
#include <QJsonDocument>
#include <QUdpSocket> // Socket to send and listen datagrams
#include <QList> // Stores connected client
#include <QPair> // To save each client as a IP and port number pair

#include "shared_elements.h" // The game state
#include "game_multiplayer_lobby_elements.h" // Loby UI hookup
...
public:
    explicit Game_Networking_Elements(QObject *parent = nullptr); //Makes the UDP socket and connects its readuread
...
private:
    void Send_Chat_Packet(const QString &senderName, const QString &message, const QHostAddress &address, quint16 port);
    void Remember_Client_Endpoint(const QHostAddress &address, quint16 port);
...
private:
    ...
    QList<QPair<QHostAddress, quint16> > Connected_Client_Endpoints; // Pair of IP and Port numbers stored in list
};
