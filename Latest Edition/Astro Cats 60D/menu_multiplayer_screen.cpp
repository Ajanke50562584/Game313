...
static const char *MULTIPLAYER_BACKGROUND_PATH = ":/Images/Background/Space_BG_2.png";
static const char *MULTIPLAYER_CHARACTER_PATH = ":/Images/Characters/Sitting_Black.png";
    ...
    , Animation_Status_Reported(false)
{
    ...
    // No animation loops have completed yet
    // The animation status has not been reported yet

    setAttribute(Qt::WA_StyledBackground, true);
    QPixmap test(MULTIPLAYER_BACKGROUND_PATH);
    qDebug() << test.isNull();

    // The background image stretches with widget.
    setObjectName("MultiplayerMenu");
    setStyleSheet(QString(
                      "#MultiplayerMenu {"
                      "border-image: url(%1) 0 0 0 0 stretch stretch;" // stretc hhorz and vert with no padding
                      "}").arg(MULTIPLAYER_BACKGROUND_PATH));

    // Sprite sheet chopper
    Sprite_Sheet = new QPixmap(MULTIPLAYER_CHARACTER_PATH);
    qDebug() << "sprite null?" << Sprite_Sheet->isNull();
    qDebug() << "sprite size:" << Sprite_Sheet->width() << Sprite_Sheet->height();

    Frames.clear(); // Clear the animation frames
    if(!Sprite_Sheet->isNull())
    {
        const int Frame_Width = Sprite_Sheet->width() / 8; // Becuz there is 8 frames
        const int Frame_Height = Sprite_Sheet->height();
   ...
   // put character in front of background
    Character_Label = new QLabel(this);
    Character_Label->setAttribute(Qt::WA_TranslucentBackground); // to see the background behind him
    Character_Label->raise();// bring to front
    ...
    // The timer calls Update_Frame every time the animation should advance.
    Multiplayer_Character_Timer = new QTimer(this);
    connect(Multiplayer_Character_Timer, SIGNAL(timeout()), this, SLOT(Update_Frame()));

    // Main navigation buttons for hosting, joining, or returning.
    ...
    // The exact button size is recalculated whenever the window changes size.
    ...
    Host_Button->setCursor(Qt::PointingHandCursor); // I hope this is worth atleast 1 awesomeness point
    ...
    connect(Host_Button, SIGNAL(clicked()), this, SIGNAL(Host_Button_Clicked()));
    connect(Client_Button, SIGNAL(clicked()), this, SIGNAL(Client_Button_Clicked()));
    connect(Back_Button, SIGNAL(clicked()), this, SIGNAL(Back_Button_Clicked()));

    //Grid for the button
    QGridLayout *Layout = new QGridLayout(this);

    // Initial spacing is replaced by responsive values in Apply_Responsive_Sizing().
    Layout->setContentsMargins(60, 45, 60, 45);// left top right bottom ,margins
    Layout->setHorizontalSpacing(35);
    Layout->setVerticalSpacing(35);

    // Equal stretch keeps each grid section sharing the available space.
    ...
    // Thanks Exam Prep for the inspo
    ...
    setLayout(Layout);
    Character_Label->raise();// Make sure el gatito is always visble
...
void Menu_Multiplayer_Screen::Update_Frame()
{
    ...
    // Scale the current frame to fit the responsive label size.
    ...
    if(Current_Frame == 0)
    {
        ...
        if(Completed_Animation_Loops >= 3 && !Animation_Status_Reported)
        {
            ...
            qDebug() << "Animation successful XD : Multiplayer menu";
        }
    }
}
...
void Menu_Multiplayer_Screen::hideEvent(QHideEvent *event)
{
    ...
    if(Multiplayer_Character_Timer != nullptr && Multiplayer_Character_Timer->isActive())
    {
        ...
        // This is mostly sothat it doesnt just loop in the bavckground eating more RAM than Google
    }
}

void Menu_Multiplayer_Screen::Apply_Responsive_Sizing()
{
    const int w = width(); // of window
    const int h = height(); // of window

    const int Button_Width = qMax(160, (260 * w) / 1280); // Take which ever one is bigger
    const int Button_Height = qMax(72, (120 * h) / 720);

    // Fixed min/max values keep each button stable after the responsive size is chosen.
    ...
    if(Grid != nullptr)
    {
        // Grid margins and gaps scale from the current window dimensions.
    ...
    if(Character_Label != nullptr)
    {
        ...
        Character_Label->resize(qMax(96, (384 * w) / 1280), qMax(96, (384 * h) / 720));
        Character_Label->move(qMax(12, (30 * w) / 1280), height() - Character_Label->height() - qMax(12, (30 * h) / 720)); // Put el Gattito noir in bottom left
        Character_Label->raise(); // to front
...
void Menu_Multiplayer_Screen::Apply_Responsive_Styles()
{
    ...
    Host_Button->setStyleSheet(QString(
                                   ...
                                   "padding: %3px;"
                                   "}").arg(Radius).arg(Host_Font_Size).arg(Padding)); // Need this for the dynamic scaling T_T
...
void Menu_Multiplayer_Screen::Report_Animation_Error_Once()//Tell me I fail onl once
{
