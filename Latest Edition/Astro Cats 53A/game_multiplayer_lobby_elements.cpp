    ...
    , Ellipsis_Frame(0)
{
    ...
    Ready_Button = new QPushButton("Not Ready", Top_Middle_Panel);
    ...
    connect(Ready_Button, &QPushButton::clicked, this, &Game_Multiplayer_Lobby_Elements::Handle_Ready_Toggled);

    Top_Middle_Layout->addStretch();
    Top_Middle_Layout->addWidget(Lobby_Name_Label);
    Top_Middle_Layout->addWidget(Ready_Button, 0, Qt::AlignHCenter);
    Top_Middle_Layout->addStretch();
...
void Game_Multiplayer_Lobby_Elements::Refresh_From_Shared_Elements()
{
    ...
    Ready_Button->setText(Get_Local_Player_Ready_State() ? "Ready" : "Not Ready");
...
void Game_Multiplayer_Lobby_Elements::Apply_Responsive_Sizing()
{
    ...
    const int Send_Button_Width = qMax(92, (108 * w) / 1280);
...
bool Game_Multiplayer_Lobby_Elements::Get_Local_Player_Ready_State() const
{
    if(Shared_Group == nullptr)
        return false;
...
void Game_Multiplayer_Lobby_Elements::Handle_Ready_Toggled()
{
    if(Shared_Group == nullptr)
        return;

    const bool Is_Ready = Get_Local_Player_Ready_State();
    Shared_Group->Set_Local_Player_Ready(!Is_Ready);
    Refresh_From_Shared_Elements();
}
