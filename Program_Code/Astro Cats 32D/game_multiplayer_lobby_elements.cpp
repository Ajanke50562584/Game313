    ...
    , Ellipsis_Frame(0)
{
    ...
    // stronger button style so the buttons actually show properly
    ...
    // temp demo rows
    // note: no dots here, dots get animated by timer
    Player_List_Layout->addWidget(Create_Player_Row(
        "Dan ~ Warning:: The Void consumes dormant stars"));
    Player_List_Layout->addWidget(Create_Player_Row(
        "Bleh ~ Rest assured, The Last Constellation envelops you"));
    ...
    // =========================================================
    // TOP MIDDLE : LOBBY NAME + CONNECTION STATUS
    // =========================================================
...
QLabel *Game_Multiplayer_Lobby_Elements::Create_Player_Row(const QString &playerLineText)
{
    ...
    // store label + base text so we can animate the dots later
...
// updates the animated dots on each player status
...
// attach shared data so this screen can refresh from it
...
// update visible data from shared state
void Game_Multiplayer_Lobby_Elements::Refresh_From_Shared_Elements()
{
    ...
    // only host should see host controls
...
// returns the main widget for this whole screen
