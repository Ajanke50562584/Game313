    ...
    , Ellipsis_Frame(0)
{
    ...
    Lobby_Widget->setStyleSheet(
        "QWidget {"
        "background-image: url(:/Images/Background/Space_BG_5.png);"
    ...
    // main grid
    Main_Layout = new QGridLayout(Lobby_Widget);
...
QLabel *Game_Multiplayer_Lobby_Elements::Create_Player_Row(const QString &playerLineText)
{
    ...
    Player_Row_Label->setMinimumHeight(qMax(18, (22 * Lobby_Widget->height()) / 720));
...
{
    if(watched == Lobby_Widget)
    {
        if(event->type() == QEvent::Resize || event->type() == QEvent::Show)
            Apply_Responsive_Sizing();

        // only animate the lobby status dots while this screen is visible
        if(event->type() == QEvent::Show && Ellipsis_Timer != nullptr && !Ellipsis_Timer->isActive())
            Ellipsis_Timer->start(500);

        if(event->type() == QEvent::Hide && Ellipsis_Timer != nullptr && Ellipsis_Timer->isActive())
            Ellipsis_Timer->stop();
    }
...
{
    if(Lobby_Widget == nullptr || Main_Layout == nullptr)
        return;

    const int w = Lobby_Widget->width();
    const int h = Lobby_Widget->height();
    ...
    Leave_Lobby_Button->setMinimumWidth(qMax(120, (170 * w) / 1280));
    Customize_Character_Button->setMinimumWidth(qMax(150, (220 * w) / 1280));
    Vote_Map_Button->setMinimumWidth(qMax(120, (170 * w) / 1280));
    Send_Chat_Button->setMinimumWidth(qMax(72, (100 * w) / 1280));
}
